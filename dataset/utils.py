...
class DomainAdaptationDataset(Dataset):
    ...
    def __getitem__(self, index):
        src_x = self.T(Image.open(self.source_examples[index][0]).convert('RGB')).to(CONFIG.dtype)
        targ_x = self.T(Image.open(random.choice(self.target_examples)[0]).convert('RGB')).to(CONFIG.dtype)
        src_y = torch.tensor(self.source_examples[index][1]).long()
        return src_x, src_y, targ_x
