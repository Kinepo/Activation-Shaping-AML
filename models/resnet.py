    ...
    def point_3(self, targ_x):
        ...
        def hook_2(module, input, output):
            if CONFIG.experiment in ['ASHResNet18_DA']:
                ...
                output = output * torch.mul(torch.where(list_of_source[-1] > 0, 1.0, 0.0), torch.where(
                    torch.isin(output, torch.topk(output.flatten(), k=CONFIG.hyper_parameter)[0]), 1.0,
                    0.0))
                list_of_source.pop()
                hook2[-1].remove()
                hook2.pop()
        ...
        def hook_2(module, input, output):
            if CONFIG.experiment in ['ASHResNet18']:
                ...
                output = output * torch.mul(torch.bernoulli(
                    torch.full(output.size(), fill_value=CONFIG.random_parameter, device=CONFIG.device)),
                    torch.where(torch.isin(output, torch.topk(output.flatten(), k=CONFIG.hyper_parameter)[0]), 1.0,
                                0.0))
                hook[-1].remove()
                hook.pop()
                return output
